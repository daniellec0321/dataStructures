...
// int partition(VECTOR<wordle_word>& section, int lo, int hi)
{

	double pivot = section[hi];
	// double pivot = section[hi].info_score;
	int i = lo-1;

	for (int j=lo; j<hi; j++) {

		// if (section[j].info_score < pivot) {
		if (section[j] < pivot) {

			i++;
			// swap here
			double temp = section[j];
			// wordle_word temp = section[j];
			section[j] = section[i];
			section[i] = temp;
...
// void quicksort(VECTOR<wordle_word>& tobeSorted, int lo, int hi)
{
...
// creates an unordered map of words and their common scores
int find_common_score(const STRING filename, UNOR_MAP<STRING, double>& common_scores)
{
	...
	while (infile >> reader) {
		...
		common_scores.insert({reader, iter});
...
int read_file(const STRING filename, UNOR_MAP<STRING, double> common_scores, VECTOR<wordle_word>& the_words)
{
	// declaring filestream
	IFSTREAM infile;
	infile.open(filename);
	...
	while (infile >> reader) {
		...
		// add its common score
		if (common_scores.find(reader) != common_scores.end()) {
			the_word.common_score = common_scores[reader];
		} else {
			COUT << reader << ENDL;
			the_word.common_score = std::numeric_limits<double>::max();
		}
...
void best_words(VECTOR<wordle_word> rec_words)
{
	...
	// if rec_words is empty
	/* if (rec_words.size() == 0) {

		COUT << "There are no possible words." << ENDL;
		return;

	} else {

		// printing how many possible words there are
		COUT << ENDL << rec_words.size() << " possible words" << ENDL;
	
	}

	// sorting rec words based on info score
	quicksort(rec_words, 0, (int)rec_words.size()-1);

	// print the top ten words
	for (unsigned int i=1; i<=10; i++) {
		
		COUT << i << ": " << rec_words[rec_words.size()-i].word << ENDL;
	...
	for (auto elem : rec_words) {
		...
		the_scores.push_back(elem.info_score);
	}
	...
	for (unsigned int rank_count=1; rank_count<=10; rank_count++)
		...
		for (unsigned int i=0; i<rec_words.size(); i++) {
			...
			if (rec_words[i].info_score == the_scores[the_scores.size()-rank_count]) {
